# 🛫 FE Integration Guide — Part 1: Setup & Foundation
...
│   └── useSearchStore.ts         ← searchParams — dùng giữa Search → Results
│
...
│           └── FlightResults.tsx ← CÓ SẴN, cần tích hợp (xóa mock data)
│
└── utils/
...
## BƯỚC 1 — `src/api/axiosInstance.ts`
...
## BƯỚC 5 — `src/store/useAuthStore.ts` (ZUSTAND)

> **Tại sao Zustand?**
> Token + thông tin user cần dùng ở Navbar, ProtectedRoute, trang Booking.
> Nếu dùng useState thì phải prop-drill rất sâu qua nhiều component.
...
## BƯỚC 6 — `src/store/useSearchStore.ts` (ZUSTAND)

> **Tại sao Zustand cho search params?**
> Khi user bấm "Sửa tìm kiếm" từ FlightResults quay về FlightSearch,
> Zustand giữ giá trị form mà không cần parse URL lại.
> SearchParams cũng cần ở FlightDetail để biết hạng ghế đang chọn.
...
## BƯỚC 7 — `src/hooks/useAuth.ts` (REACT QUERY MUTATIONS)

> **Tại sao useMutation?**
> Login/Register/ForgotPassword là thao tác **ghi** (POST), không phải đọc.
> `useMutation` xử lý loading/error/success tự động, không cache như `useQuery`.
...
## BƯỚC 8 — `src/hooks/useFlights.ts` (REACT QUERY QUERIES)
